...
mateDataCreateTaskHourMinuteSecond=23-09-20
workDir=/Users/fuyuguang/jiuxian_studio/doc/1版本/酒仙官网app/9.1.20/2023-11-28--17-18-09_付玉光___feat_sfwf/子任务/2023-11-28--17-33-14_付玉光___feat_我的/子任务/2023-11-28--23-09-20_付玉光___feat_诉讼费
branchName=feature/feature_fyg_9.1.20_诉讼费
preReleaseBranchName=feature/feature_preRelease_9.1.20
renamedFeatureBranchName=
createTaskTime=2023-11-28/23-09-37
...
#taskDesc=table-layout:fixed;列宽 = 表格总宽度 / 列数。首行决定列宽。(列宽平均分)  table-layout:automatic; 列宽以内容最长的单元格决定。(列宽,内容长的长,内容短的短
#changeScope=您的应用于应能适应这些模型。 在某些情况下（例如对于 Wear OS），您应重新考虑应用的用户体验并构建专用于该设备的应用。 为支持 Chrome 操作系统设备（例如 Google Pixelbook），您可能只需对现有应用进行微改，即可支持键盘/鼠标交互和更大的屏幕。如要支持这些设备，请参阅以下开发者指南：构建适用于 Wear OS 的应用 构建适用于 Android TV 的应用 构建适用于 Android Auto 的应用 构建适用于 Chrome 操作系统的应用 幕构建应用的具体说明，请参阅以下页面：支持不同的屏幕尺寸  支持不同的像素密度
