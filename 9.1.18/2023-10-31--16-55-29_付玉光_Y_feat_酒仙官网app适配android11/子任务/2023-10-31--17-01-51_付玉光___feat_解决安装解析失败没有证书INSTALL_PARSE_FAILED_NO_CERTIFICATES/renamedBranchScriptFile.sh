...
if [ -d "${workDir}/子任务" ]; then
      ...
      if [ -n "$(ls -A "${workDir}/子任务")" ]; then
              ...
              while read -r subTaskMetaDataFileItem; do
                  ...
                  #新的工作目录=新的/Users/fuyuguang/jiuxian_studio/doc/1版本/酒仙官网app/9.1.18/2023-10-31--16-55-29_付玉光_Y_feat_酒仙官网app适配android11/子任务/2023-10-31--17-01-51_付玉光___feat_解决安装解析失败没有证书INSTALL_PARSE_FAILED_NO_CERTIFICATES/子任务 + 子任务工作目录名
