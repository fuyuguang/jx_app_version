...
#  cd "${currentDir}"
  ...
  taskHierarchyNameAbbreviation=$(getMetaDataFileSingleProperty "${metaDataFile}" "taskHierarchyNameAbbreviation")
  #项目元数据列出版本目录文件存放路径
...
if [ -d "${workDir}/子任务" ]; then
      ...
      if [ -n "$(ls -A "${workDir}/子任务")" ]; then
              ...
              while read -r subTaskMetaDataFileItem; do
                  ...
                  #新的工作目录=新的/Users/fuyuguang/jiuxian_studio/doc/1版本/酒仙官网app/9.1.18/2023-11-22--10-31-49_付玉光___feat_折叠屏适配_我的酒仙页面_福利中心图片显示不全/子任务 + 子任务工作目录名
