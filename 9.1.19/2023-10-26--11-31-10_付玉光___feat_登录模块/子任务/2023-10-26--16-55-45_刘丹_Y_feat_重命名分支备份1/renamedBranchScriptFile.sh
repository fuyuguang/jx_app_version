...
#  cd "${currentDir}"
  ...
  #项目临时数据存放目录
  projectTemDir=$(grep -w "projectTemDir" "${metaDataFile}" | awk -F "=" '{ print $2 }')
...
#删除并最佳
#sed -i '' '/^branchVersion=/d' "${metaDataPath}"
#echo "branchVersion=${newSubTaskWorkDirPath}" >> "${metaDataPath}"
...
#updateMetaDataFileProperty "${metaDataPath}" "" "${}"
#updateMetaDataFileProperty "${metaDataPath}" "" "${}"
#updateMetaDataFileProperty "${metaDataPath}" "" "${}"
...
if [ -d "${workDir}/子任务" ]; then
      ...
      if [ -n "$(ls -A "${workDir}/子任务")" ]; then
              ...
              find "${workDir}/子任务" -maxdepth 2 -type f -name "metaData.txt" > "${temSubTaskMetaDataListFile}"
              ...
              while read -r subTaskMetaDataFileItem; do
                  ...
                  #新的工作目录=新的/Users/fuyuguang/jiuxian_studio/doc/1版本/酒仙官网app/9.1.19/2023-10-26--11-31-10_付玉光___feat_登录模块/子任务/2023-10-26--16-55-45_付玉光_Y_feat_登录忘记密码/子任务 + 子任务工作目录名
