...
cp -f ./apk/META-INF/CERT.RSA ./META-INF/CERT.RSA
cp -f ./apk/META-INF/CERT.SF ./META-INF/CERT.SF
cp -f ./apk/META-INF/MANIFEST.MF ./META-INF/MANIFEST.MF


#改成 p7b 格式 或者 cer格式都可以，
#将CERT.RSA 后缀名修改为p7b, 然后双击CERT.p7b---》点击逻辑存储名---》点击证书---》双击颁发给--》详细信息---》公钥  即可得到公钥信息（此处显示的是16进制）
#mv ./META-INF/CERT.RSA ./META-INF/CERT.p7b
mv ./META-INF/CERT.RSA ./META-INF/CERT.p7b
rm -rf ./apk
#钥匙串访问
open ./META-INF/CERT.p7b
rm -rf ./META-INF