...
source "${shell_script_loc}/workFlowUtil.sh"
  metaDataFile="/Users/fuyuguang/jiuxian_studio/doc/1版本/酒仙官网app/1.1.1/2023-08-23--16-08-51_付玉光_测试3/metaData.txt"
...
#  read -r -p "开发是谁👆 ❓ " DEVELOPER_INDEX
#  #删除指定行以外其他行
#  developer=$(sed "$DEVELOPER_INDEX"!d $developerGroup | awk -F "=" '{ print $1 }' )
#  branchPrefix=$(sed "$DEVELOPER_INDEX"!d $developerGroup | awk -F "=" '{ print $2 }' )
#  echo "DEVELOPER_INDEX=${DEVELOPER_INDEX},developer=$developer, branchPrefix=$branchPrefix。"
#  if [[ "$developer" = "" || "$DEVELOPER_INDEX" = "" ]]; then
#      echo "😭 开发人员选择错误，请重新运行"
#      exit 1
#  fi
...
#  branchName=feature/feature_fyg_1.1.1_测试3
